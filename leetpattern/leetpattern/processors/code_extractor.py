...
class CodeExtractor:
    ...
    def extract_algorithm_code(self, problem: Problem) -> str:
        ...
        # Add Python solution
        if problem.python_path and self.file_manager.file_exists_and_not_empty(problem.python_path):
        ...
        # Add C++ solution 
        if problem.cpp_path and self.file_manager.file_exists_and_not_empty(problem.cpp_path):
    ...
    def extract_sql_code(self, problem: Problem) -> str:
        ...
        if problem.txt_path and self.file_manager.file_exists_and_not_empty(problem.txt_path):
            basename = problem.basename or str(problem.qid)
            content += f'```txt title="{title}"\n--8<-- "solutions/sql/{basename}.txt"\n```\n\n'
        
        # Add SQL solution
        if problem.sql_path and self.file_manager.file_exists_and_not_empty(problem.sql_path):
            basename = problem.basename or str(problem.qid)
